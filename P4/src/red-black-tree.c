...
//Funcions per copiar un arbre
void copiar(RBTree *tree,Node *x){
  RBData *n;
  n = findNode(tree,x->data->key); 

  if(n != NULL){
    n->num_vegades = n->num_vegades + x->data->num_vegades;
  }else{
    insertNode(tree,x->data);
  }
}
void copiarArbreRecursiu(RBTree *tree,Node *x){
  copiar(tree,x);
  if (x->right != NIL){
    copiarArbreRecursiu(tree,x->right);

  }

  if (x->left != NIL){
    copiarArbreRecursiu(tree,x->left);
  }
}

void copiarArbre(RBTree *tree,RBTree *treeACopiar){
  if(treeACopiar->root!=NIL){
    copiarArbreRecursiu(tree,treeACopiar->root);
  }
